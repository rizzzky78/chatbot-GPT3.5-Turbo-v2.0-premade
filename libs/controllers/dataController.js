 ...
 * }>
 */
const getUserData = async (TuserNumber) => {
  const userData = await collections.userData.findOne({ userNumber: TuserNumber });
  const {userID, userNumber, userName, token, totalUsage, sessionChat, dataChat} = userData
  return {
    userID, // string
    userNumber, // string
    userName, // string
